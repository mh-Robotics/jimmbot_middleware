...
void MCP2515::startSPI() { PORTB &= ~(1 << SPICS); }

void MCP2515::endSPI() { PORTB |= (1 << SPICS); }

MCP2515::ERROR MCP2515::reset(void) {
  startSPI();
  spi_writereadbyte(INSTRUCTION_RESET);
  endSPI();
...
uint8_t MCP2515::readRegister(const REGISTER reg) {
  startSPI();
  spi_writereadbyte(INSTRUCTION_READ);
  spi_writereadbyte(reg);
  uint8_t ret = spi_writereadbyte(0x00);
  endSPI();
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[],
  ...
  for (uint8_t i = 0; i < n; i++) {
    values[i] = spi_writereadbyte(0x00);
  }
  endSPI();
}
...
void MCP2515::setRegister(const REGISTER reg, const uint8_t value) {
  startSPI();
  spi_writereadbyte(INSTRUCTION_WRITE);
  spi_writereadbyte(reg);
...
void MCP2515::modifyRegister(const REGISTER reg, const uint8_t mask,
                             const uint8_t data) {
  startSPI();
  spi_writereadbyte(INSTRUCTION_BITMOD);
  spi_writereadbyte(reg);
  spi_writereadbyte(mask);
...
uint8_t MCP2515::getStatus(void) {
  startSPI();
  spi_writereadbyte(INSTRUCTION_READ_STATUS);
  uint8_t i = spi_writereadbyte(0x00);
  endSPI();
...
MCP2515::ERROR MCP2515::setMode(const CANCTRL_REQOP_MODE mode) {
  ...
  unsigned long endTime = ::millis_get() + 10;
  bool modeMatch = false;
  while (::millis_get() < endTime) {
...
void MCP2515::clearMERR() { modifyRegister(MCP_CANINTF, CANINTF_MERRF, 0); }

void MCP2515::clearERRIF() { modifyRegister(MCP_CANINTF, CANINTF_ERRIF, 0); }
